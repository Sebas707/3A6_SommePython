...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('-d', '--délai', metavar='DÉLAI', type=float, default=2, help='Délai pour le calcul (défaut 2 sec)')
...
def verifier_erreur(délai_exécution: float) -> None:
    if délai_exécution <= 0:
        print(Fore.YELLOW + "[SF] " + Fore.RED + "ValueError" + Fore.YELLOW + ": Le délai doit être supérieur à 0" + Fore.RESET)
        exit(1)
    if délai_exécution > 5:
        print(Fore.YELLOW + "[SF] " + Fore.RED + "ValueError" + Fore.YELLOW + ": Le délai doit être au plus 5 secondes" + Fore.RESET)
        exit(1)
...
def main() -> None:
    ...
    try:
        evaluation = eval(' '.join(args.code) or "None")
